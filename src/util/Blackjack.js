...
/** 
    ...
    * @version 4.0.0
    * @link https://discord-blackjack.gitbook.io/discord-blackjack
...
module.exports = async (message, options) => {
    ...
    if (commandType === "interaction" && !message.deferred && !message.replied) {
        ...
        starterMessage = await message.reply({ embeds: [{ title: "Game is starting.", description: "The game is starting soon, get ready!" }] })
    }

    // check if the user is playing a game
    if (games.has(message.member.id)) {
        if (commandType === "message") {
            message.reply("You are already playing a blackjack game!")
        } else if (commandType === "interaction") {
            if (message.replied || message.deferred) {
                message.followUp({ content: "You are already playing a blackjack game!" })
            } else {
                message.reply({ content: "You are already playing a blackjack game!" })
            }
        }
        return {
            result: "None",
    ...
    // set the embeds
    let winEmbed = { title: "You won!", color: 0x008800, description: "", fields: [], footer: { text: `You win ${Util.formatNumber(bet)} Monkey!`, icon_url: message.member.user.displayAvatarURL() } }
    let loseEmbed = { title: "You lost!", color: 0xFF0000, description: "", fields: [], footer: { text: `You lost ${Util.formatNumber(bet)} Monkey :(`, icon_url: message.member.user.displayAvatarURL() } }
    let tieEmbed = { title: "It's a tie.", color: 0xFFFF00, description: "", fields: [], footer: { text: "You got your monkey back.", icon_url: message.member.user.displayAvatarURL() } }
    ...
    let timeoutEmbed = { title: "Time's up!", color: 0xFF0000, description: "You took more than 30 seconds to respond. The time is up and the game has canceled.", fields: [], footer: { text: "You got your monkey back.", icon_url: message.member.displayAvatarURL() } } 
    let cancelEmbed = { title: "Game canceled.", color: 0xFF0000, description: "You decided to cancel your ongoing blackjack game.", fields: [], footer: { text: "You got your monkey back.", icon_url: message.member.displayAvatarURL() } }
    let generalEmbed = normalEmbed === false ? options.normalEmbedContent : { title: "Blackjack", color: Math.floor(Math.random() * (0xffffff + 1)), fields: [{ name: "Your hand", value: "", inline: true }, { name: `Dealer's hand`, value: "", inline: true }], footer: { text: `You bet ${Util.formatNumber(bet)} Monkey`, icon_url: message.member.user.displayAvatarURL() } }
    ...
    if (yourvalue === 21 && dealervalue != 21) {
        if (options.resultEmbed === true) {
            ...
            commandType === "message" ? starterMessage.edit({ embeds: [winEmbed] }) : message.editReply({ embeds: [winEmbed], allowedMentions: { repliedUser: true } })
        }
    ...
    // }
        ...
        if (options.resultEmbed === true) {
            ...
            commandType === "message" ? starterMessage.edit({ embeds: [tieEmbed] }) : message.editReply({ embeds: [tieEmbed], allowedMentions: { repliedUser: true } })
        }
    ...
    const editReply = async (msg, reply, commandType) => {
        if (commandType === "message") {
            ...
            return await message.editReply({ embeds: [reply], components: buttons ? [row1, row2] : [], allowedMentions: { repliedUser: true } })
        }
    }
    ...
    if (options.resultEmbed === true) {
        ...
        options.commandType === "message" ? currentMessage.edit({ embeds: [finalEmbed] }) : message.editReply({ embeds: [finalEmbed], allowedMentions: { repliedUser: true } })
